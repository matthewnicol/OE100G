...
// Definitions for function types and function pointers we will load from
// opengl32.dll
...
// Definitions for attributes for specifying the version we want to
// wglCreateContextAttribsARB
...
// Definitions for attributes for specifying the pixel format we want to
// wglChoosePixelFormatARB
...
void *OS_loadOpenGLFunction(const char *name) {
  ...
  if (p == 0 || (p == (void *)0x1) || (p == (void *)0x2) ||
      (p == (void *)0x3) || (p == (void *)-1)) {
    // Functions added to OpenGL after version 1.1 can't be loaded with
    // wglGetProcAddress. Use GetProcAddress if wglGetProcAddress doesn't
    // work.
...
// For now, we have one active window, and its attributes are stored in this
// global struct. If this ever changes, a lot of this code will need to be
// restructured.
...
LRESULT CALLBACK SystemEventCallback(HWND hwnd, UINT message, WPARAM wparam,
                                     LPARAM lparam) {
  switch (message) {
  // WINDOW LIFECYCLE
  case WM_CLOSE: {
...
void NBASE_OSWindow_handleEvents() {
  // Our engine will call this function once per frame (for now... this might
  // change later).
...
void NBASE_OSWindow_startup(S32 width, S32 height) {
  ...
  HWND window =
      createWindow(hInstance, width, height, bitsPerPixel, isFullScreen);
  HDC gldc = GetDC(window);
  initialiseOpenGL(gldc);
...
HGLRC initialiseOpenGL(HDC bindToDC) {
  {
    ...
    // create a proper OpenGL context below.
    ...
    ASSERT_WITH_MESSAGE(RegisterClassA(&legacyWindowClass),
                        "Bootstrapping OpenGL: Failed to register Legacy "
                        "OpenGL Window Class.");

    HWND legacyOpenGLWindow =
        CreateWindowExA(0, legacyWindowClass.lpszClassName, "LEGACY WINDOW", 0,
                        CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT,
                        CW_USEDEFAULT, 0, 0, legacyWindowClass.hInstance, 0);

    ASSERT_WITH_MESSAGE(legacyOpenGLWindow,
    ...
    S32 pixelFormat =
        ChoosePixelFormat(legacyDeviceContext, &desiredPixelFormat);
    ASSERT_WITH_MESSAGE(pixelFormat, "Failed to find a suitable pixel format.");
    ASSERT_WITH_MESSAGE(
  ...
  // wglChoosePixelFormatARB.
  int desiredPixelFormatAttributes[] = {WGL_DRAW_TO_WINDOW_ARB,
                                        GL_TRUE,
  ...
  S32 pixelFormat;
  U32 numFormats;
  wglChoosePixelFormatARB(bindToDC, desiredPixelFormatAttributes, 0, 1,
  ...
  ASSERT_WITH_MESSAGE(SetPixelFormat(bindToDC, pixelFormat, &pfd),
  ...
  HGLRC openglRenderingContext =
      wglCreateContextAttribsARB(bindToDC, 0, desiredOpenGLVersionAttributes);

  ASSERT_WITH_MESSAGE(openglRenderingContext,
                      "Failed to create OpenGL context.");

  ASSERT_WITH_MESSAGE(wglMakeCurrent(bindToDC, openglRenderingContext),
...
HWND createWindow(HINSTANCE inst, S32 width, S32 height, S32 bitsPerPixel,
  ...
  ASSERT_WITH_MESSAGE(RegisterClassA(&window_class),
  ...
  HMONITOR monitor =
      MonitorFromPoint((const POINT){0, 0}, MONITOR_DEFAULTTOPRIMARY);
  MONITORINFO monitorInfo = {sizeof(monitorInfo)};
  GetMonitorInfo(monitor, &monitorInfo);

  // Position our window such that it is in the middle of the default monitor's
  // screen
  ...
  if (rect.right < monitorInfo.rcMonitor.right - monitorInfo.rcMonitor.left) {
    rect.left = (monitorInfo.rcMonitor.right - monitorInfo.rcMonitor.left -
                 rect.right) /
                2;
  ...
  if (rect.bottom < monitorInfo.rcMonitor.bottom - monitorInfo.rcMonitor.top) {
    rect.top = (monitorInfo.rcMonitor.bottom - monitorInfo.rcMonitor.top -
                rect.bottom) /
               2;
  ...
  DWORD windowStyle =
      WS_POPUP | WS_CLIPSIBLINGS | WS_CLIPCHILDREN | WS_TILEDWINDOW;

  AdjustWindowRect(&rect, WS_EX_TOPMOST, 0);

  HWND window = CreateWindowEx(WS_EX_TOPMOST, window_class.lpszClassName,
                               NBASE_WINDOW_NAME_QUOTED, windowStyle, 0, 0,
                               rect.right - rect.left, rect.bottom - rect.top,
                               0, 0, inst, 0);

  ASSERT_WITH_MESSAGE(window, "Failed to create window.");

  SetWindowPos(window, 0, rect.left, rect.top, 0, 0,
...
void NBASE_OSWindow_swapBuffers() { SwapBuffers(OS_activeWindow.hDC); }
